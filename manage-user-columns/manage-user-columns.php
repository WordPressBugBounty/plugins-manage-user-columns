	...
	function dpk_muc_modify_user_table_row( $output, $column_id, $uid ) {
		...
		if($muc_cc){
			if(array_key_exists($column_id, $muc_cc)){
				...
				{
					$custom_col_val = $user_meta_val;
				} 
				else
				{
					$custom_col_val = http_build_query($user_meta_val,'',', ');
				}
				
				$custom_col_val = apply_filters('muc_filter_custom_col_val', $custom_col_val, $column_id, $uid );
				
				return $custom_col_val;
